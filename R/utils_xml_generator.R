...
generate_synthetic_xml <- function(schema_data, cds_code,
  ...
  for (i in seq_len(nrow(elements))) {
    ...
    existing <- xml2::xml_find_first(parent_node,
                                      glue::glue("./{el_name}"))
...
placeholder_value <- function(type_name, schema_data) {
  ...
  if (nrow(type_info) == 0L) return("PLACEHOLDER")

  base <- type_info$base_type[[1]] %||% ""

  if (stringr::str_detect(base, "date")) return(as.character(Sys.Date()))
  if (stringr::str_detect(base, "time")) return("09:00:00")
  if (stringr::str_detect(base, "integer|decimal")) return("1")
